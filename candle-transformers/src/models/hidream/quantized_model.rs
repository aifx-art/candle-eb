use super::model::{timestep_embedding, EmbedNd};
...
impl SelfAttention {
    ...
    fn forward(&self, xs: &Tensor, pe: &Tensor) -> Result<Tensor> {
        ...
        super::model::attention(&q, &k, &v, pe)?.apply(&self.proj)
    }
}
...
impl DoubleStreamBlock {
    fn new(cfg: &super::Config, vb: VarBuilder) -> Result<Self> {
    ...
    fn forward(
        ...
        let attn = super::model::attention(&q, &k, &v, pe)?;
...
pub trait WithForward {
    ...
    fn forward_with_cfg(
        ...
        candle::bail!("forward_with_cfg not implemented for this model")
    }
}
...
impl HDQuantizedModel {
    pub fn new(cfg: &super::Config, vb: VarBuilder) -> Result<Self> {
        ...
        let x_embedder = linear(cfg.in_channels, inner_dim, vb.pp("x_embedder"))?;
        ...
        let final_layer = LastLayer::new(inner_dim, cfg.patch_size, cfg.out_channels, vb.pp("final_layer"))?;
        let pe_dim = inner_dim / cfg.num_heads;
...
impl WithForward for HDQuantizedModel {
    ...
    fn forward(
        ...
        let vec_ = timestep_embedding(timesteps, 256, dtype)?.apply(&self.t_embedder)?;
        let vec_ = match guidance {
            Some(guidance) => {
                (vec_ + timestep_embedding(guidance, 256, dtype)?)?
            }
            _ => vec_,
        };
        let vec_ = (vec_ + y.apply(&self.p_embedder))?;
